...
data.mt = 2;
data.l = 1;
data.J = 0*eye(3);                % pendulum inertia w.r.t body-frame
...
spherical.ph1(i) = rad2deg(atan2(-X_s2(i,2),X_s2(i,3)));
spherical.th1(i) = rad2deg(atan2(X_s2(i,1),sqrt(X_s2(i,2)^2+X_s2(i,3)^2))); 
spherical.ph2(i) = rad2deg(atan2(-X_s2(i,5),X_s2(i,6)));
spherical.th2(i) = rad2deg(atan2(X_s2(i,4),sqrt(X_s2(i,5)^2+X_s2(i,6)^2)));
...
euler.ph1(i) = rad2deg(wrapToPi(X_eu(i,1)));
euler.th1(i) = rad2deg(wrapToPi(X_eu(i,2)));
euler.ph2(i) = rad2deg(wrapToPi(X_eu(i,3)));
euler.th2(i) = rad2deg(wrapToPi(X_eu(i,4)));
...
[special.ph1(i),special.th1(i),~] = Rot2Eul(special.R1(:,:,i),'euler');
[special.ph2(i),special.th2(i),~] = Rot2Eul(special.R2(:,:,i),'euler');
disp(i)
...
%% Energy in S^2
spherical.KE(i) = 0.5*(m1*(l1^2/4)*(spherical.dq1(:,i)'*spherical.dq1(:,i)) + ...
        mt*l1^2*(spherical.dq1(:,i)'*spherical.dq1(:,i)) + ...
        m2*(l1*spherical.dq1(:,i)+(l2/2)*spherical.dq2(:,i))'*(l1*spherical.dq1(:,i)+(l2/2)*spherical.dq2(:,i))); 
spherical.PE(i) = m1*g*(l1/2)*spherical.q1(:,i)'*e3 + mt*g*l1*spherical.q1(:,i)'*e3 +...
              m2*g*(l1*spherical.q1(:,i)+(l2/2)*spherical.q2(:,i))'*e3 ;
...
%% Energy in SO(3)
special.KE(i) = 0.5*(m1*(l1^2/4)*(special.dq1(:,i)'*special.dq1(:,i)) + ...
        mt*l1^2*(special.dq1(:,i)'*special.dq1(:,i)) + ...
        m2*(l1*special.dq1(:,i)+(l2/2)*special.dq2(:,i))'*(l1*special.dq1(:,i)+(l2/2)*special.dq2(:,i))); 
special.PE(i) = m1*g*(l1/2)*special.q1(:,i)'*e3 + mt*g*l1*special.q1(:,i)'*e3 +...
              m2*g*(l1*special.q1(:,i)+(l2/2)*special.q2(:,i))'*e3 ;
...
title('q1(3)')
subplot(4,1,4)
plot(T,vecnorm(euler.q1), T,vecnorm(spherical.q1(3,:)), T,vecnorm(special.q1(3,:)))
title('q1(3)')
legend('Euler','S^2','SO(3)')
